extends Skeleton2D
class_name PalluPhysics_Verlet
...
# Grapple mode (Phase 1: Pull-to-Point)
enum PalluMode {TRAILING, GRAPPLING}
var current_mode: PalluMode = PalluMode.TRAILING
...
func _physics_process(delta: float) -> void:
	...
	for i in range(bones.size()):
		if i == 0:
			...
			# GRAPPLE MODE: Lock tip bone to grapple anchor
			if current_mode == PalluMode.GRAPPLING and i == bones.size() - 1:
				bone_positions[i] = grapple_anchor
				bone_prev_positions[i] = grapple_anchor
			else:
	...
	for iteration in range(3): # Multiple passes for stability
		for i in range(1, bones.size()):
			# Skip tip bone in grapple mode (it's locked to anchor)
			if current_mode == PalluMode.GRAPPLING and i == bones.size() - 1:
				continue
...
func set_grapple_mode(enabled: bool, anchor: Vector2 = Vector2.ZERO) -> void:
	"""Switch between trailing and grappling modes (Phase 1)"""
	if enabled:
		current_mode = PalluMode.GRAPPLING
		grapple_anchor = anchor
		print("[PalluPhysics] Grapple mode ENABLED at ", anchor)
	else:
		current_mode = PalluMode.TRAILING
		print("[PalluPhysics] Grapple mode DISABLED - returning to trailing")
